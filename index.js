  ...
  const response = await axios.post(
    'https://api.anthropic.com/v1/messages',
    {
      ...
      messages: [
        {
          role: 'user',
          content: `راجع وحسّن السكربت مع الحفاظ على العمق والتفاصيل:
...
المطلوب:
1. صحّح أي أخطاء في الأرقام (حسب نتيجة التحقق)
2. حافظ على كل التفاصيل والشروحات الموجودة
3. حسّن الصياغة بس - ما تشيلش معلومات
4. تأكد إن كل رقم له شرح لتأثيره
5. Style: ${styleTemplate.name}
6. ${langConfig.prompt}

مهم: لا تختصر المحتوى - نفس الطول والعمق، بس حسّن الصياغة وصحّح الأخطاء.
