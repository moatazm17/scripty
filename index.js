  ...
  const response = await axios.post(
    'https://api.anthropic.com/v1/messages',
    {
      ...
      messages: [
        {
          role: 'user',
          content: `أنت خبير في كتابة Hooks قوية لفيديوهات Short. اكتب 3 hooks مختلفة عن "${topic}".
...
═══════════════════════════════════════
📝 أمثلة على التطبيق (للإلهام فقط):
═══════════════════════════════════════
مثال 1: "17 مليار دولار في الصعيد... إزاي الصين لقت الفرصة اللي مصر كانت مستنياها؟"
→ [رقم ضخم] + [سؤال] + [تناقض يثير الفضول]

مثال 2: "480 ميجاواط - ده يكفي نص مليون بيت... ليه السعودية عملت ده دلوقتي بالظبط؟"
→ [رقم] + [مقارنة] + [سؤال توقيت]

مثال 3: "المركز 25 عالمياً بعد ما كنا 37... والأغرب إن ده حصل في سنتين بس!"
→ [إنجاز] + [تناقض مع الماضي] + [عنصر مفاجأة]

═══════════════════════════════════════
⚠️ ممنوع:
═══════════════════════════════════════
- "خبر عاجل"، "لو قلتلك"، "محدش هيصدق"، "هتتصدم"
- عبارات مستهلكة أو كليشيهات
- hooks طويلة أكتر من 15 كلمة

═══════════════════════════════════════

اكتب 3 hooks مختلفة، كل واحد بأسلوب مختلف (استخدم المبادئ فوق، مش الأمثلة الحرفية):
